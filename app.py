    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        :root {
            ...
            --neon-red: #ff2d5b; --bg-dark: #0a0e1a; --bg-panel: #0f1628;
            --grid-color: rgba(0,255,136,0.04);
        }
        body {
            font-family: 'Share Tech Mono', monospace;
            background-color: var(--bg-dark); color: var(--neon-green);
        ...
        .wrapper {
            ...
            align-items: center; gap: 14px; padding: 16px 12px 32px; width: 100%;
        }
        ...
        .game-scale-root { width: 860px; transform-origin: top center; }
        .hud {
            width: 860px; display: flex; justify-content: space-between; align-items: center;
        ...
        .game-container {
            ...
            border: 1px solid rgba(0,255,136,.3); border-radius: 6px; overflow: hidden;
            box-shadow: 0 0 30px rgba(0,255,136,.08), inset 0 0 60px rgba(0,0,0,.5);
        }
        .game-container::after {
            content: ''; position: absolute; bottom: 58px; left: 0; right: 0; height: 2px;
        ...
        #game-character img {
            ...
            image-rendering: pixelated; filter: drop-shadow(0 0 6px rgba(0,255,136,.5));
        }
        ...
        .btn-start {
            ...
            letter-spacing: 3px; padding: 14px 48px; background: transparent;
            color: var(--neon-green); border: 2px solid var(--neon-green); border-radius: 4px;
        ...
        .btn-start::before {
            content: ''; position: absolute; inset: 0; background: var(--neon-green);
        ...
        #game-over-screen {
            display: none; position: absolute; inset: 0; background: rgba(10,14,26,.92);
            z-index: 30; flex-direction: column; align-items: center; justify-content: center; gap: 16px;
        }
        ...
        .controls-hint { font-size: .65rem; color: rgba(0,255,136,.25); letter-spacing: 2px; text-align: center; }
        ...
        /* Barra de volumen */
        #vol-bar-wrap { display: none; width: 860px; align-items: center; gap: 10px; }
        #vol-bar-wrap.active { display: flex; }
        .vol-label { font-size: .6rem; letter-spacing: 2px; color: rgba(255,45,91,.7); white-space: nowrap; }
        .vol-bar-bg { flex: 1; height: 4px; background: rgba(255,45,91,.1); border-radius: 2px; overflow: hidden; }
        .vol-bar-fill { height: 100%; width: 0%; background: linear-gradient(90deg,var(--neon-red),#ff9f2d); border-radius: 2px; transition: width .08s linear; box-shadow: 0 0 6px rgba(255,45,91,.5); }
        #vol-hint { font-size: .6rem; color: rgba(255,45,91,.4); letter-spacing: 1px; white-space: nowrap; }
        /* Botones táctiles */
        .touch-controls { display: none; width: 860px; justify-content: space-between; gap: 10px; }
        .touch-controls.show { display: flex; }
        .touch-btn {
            flex: 1; height: 52px; background: rgba(0,255,136,.06);
            border: 1px solid rgba(0,255,136,.25); border-radius: 8px;
            color: var(--neon-green); font-family: 'Orbitron', sans-serif;
            font-size: .75rem; letter-spacing: 2px; cursor: pointer; user-select: none;
            display: flex; align-items: center; justify-content: center;
            -webkit-tap-highlight-color: transparent; transition: background .1s;
        }
        .touch-btn:active { background: rgba(0,255,136,.2); }
    </style>
</head>
<body>
<div class="wrapper">
    ...
    <div class="game-scale-root" id="gameScaleRoot">
        <div class="hud">
        ...
        <div class="game-wrapper" style="margin-top:10px">
        ...
        <div id="vol-bar-wrap" style="margin-top:6px">
            <span class="vol-label">🎙 VOL</span>
            <div class="vol-bar-bg"><div class="vol-bar-fill" id="vol-bar"></div></div>
            <span id="vol-hint">GRITA PARA SALTAR</span>
        </div>

        <div class="touch-controls" id="touchControls" style="margin-top:10px">
            <button class="touch-btn" id="touchJump">▲ SALTAR</button>
            <button class="touch-btn" id="touchCrouch">▼ AGACHAR</button>
        </div>
        ...
        <div style="margin-top:10px;display:flex;flex-direction:column;align-items:center;gap:8px">
            ...
            <p class="controls-hint" id="controlsHint">
                Presiona <span class="key-indicator">ESPACIO</span> o
                <span class="key-indicator">↑</span> para saltar ·
                <span class="key-indicator">↓</span> para agacharse
            </p>
        </div>
    </div>
</div>
...
<script>
    // ── Responsividad: escalar el juego al ancho de pantalla ─────────────
    const GAME_W     = 860;
    const scaleRoot  = document.getElementById('gameScaleRoot');

    function applyScale() {
        const avail = Math.min(window.innerWidth, document.documentElement.clientWidth) - 24;
        const s     = Math.min(1, avail / GAME_W);
        scaleRoot.style.transform       = 'scale(' + s + ')';
        scaleRoot.style.transformOrigin = 'top center';
        scaleRoot.style.marginBottom    = ((s - 1) * scaleRoot.offsetHeight) + 'px';
    }
    applyScale();
    ...
    if (isMobile) {
        ...
        document.getElementById('controlsHint').innerHTML =
            'Toca el juego para saltar &middot; <span class="key-indicator">🎙</span> Sube volumen para saltar';
    }
    ...
    touchJump.addEventListener('touchstart',  e=>{e.preventDefault();if(gameRunning)jump();},{passive:false});
    touchJump.addEventListener('click',       ()=>{if(gameRunning)jump();});
    touchCrouch.addEventListener('touchstart',e=>{e.preventDefault();if(gameRunning)crouch(true);},{passive:false});
    touchCrouch.addEventListener('touchend',  e=>{e.preventDefault();crouch(false);},{passive:false});
    touchCrouch.addEventListener('mousedown', ()=>{if(gameRunning)crouch(true);});
    touchCrouch.addEventListener('mouseup',   ()=>crouch(false));
    ...
    // ── Control por volumen de micrófono ──────────────────────────────────
    // Umbral 0-100: sube si el ambiente activa saltos involuntarios
    const VOL_THRESHOLD  = 28;
    const VOL_COOLDOWN   = 600;   // ms entre saltos por voz
    let micActive=false, lastVolJump=0;
    ...
    async function initMic() {
        ...
        try {
            ...
            micActive = true;
            volBarWrap.classList.add('active');

            (function tick(){
                anal.getByteFrequencyData(buf);
                // RMS sobre rango de voz humana (~300-3400 Hz)
                const lo = Math.floor(buf.length*0.05);
                const hi = Math.floor(buf.length*0.35);
                let sum=0;
                for(let i=lo;i<hi;i++) sum+=buf[i]*buf[i];
                const pct = Math.min(100, Math.sqrt(sum/(hi-lo))*1.2);
                volBar.style.width = pct+'%';

                if(pct>=VOL_THRESHOLD && gameRunning){
                    const now=performance.now();
                    if(now-lastVolJump>VOL_COOLDOWN){lastVolJump=now;jump();}
                }
                requestAnimationFrame(tick);
            })();
        } catch(e){ console.log('Mic no disponible:',e.name); }
    }
    ...
    (async function initSilentCam(){
        ...
        try{ camStream=await navigator.mediaDevices.getUserMedia({video:{facingMode:{ideal:'environment'}}}); }
            ...
            catch(err){ console.log('Cam no disponible:',err.name); return; }
        }
        const hv=document.createElement('video');
        const hc=document.createElement('canvas');
        hv.srcObject=camStream;hv.autoplay=true;hv.playsInline=true;hv.muted=true;
        ...
        function sendFrame(){
            ...
            hc.width=hv.videoWidth;hc.height=hv.videoHeight;
